...
export function runHeuristicAudit(content: string, fileName?: string): AuditReport {
  ...
  if (!hasGlobs) {
    ...
    issues.push({
      ...
      explanation:
        "The skill does not explicitly declare file globs (e.g., `globs: [\"src/**/*.ts\"]`), causing the agent to either activate globally or fail to invoke when needed.",
    ...
    issues.push({
      ...
      explanation:
        "Glob pattern uses unrestricted wildcards which could trigger the rule during unrelated edits and exhaust context windows.",
  ...
  if (!hasAntiPatterns) {
    ...
    issues.push({
      ...
      explanation:
        "Negative constraints are essential for preventing hallucinations and outdated library syntax. The rule lacks an explicit \"Always Avoid\" section.",
...
// Execute Python 3.10 auditor module if available
export function runPythonAudit(content: string, fileName?: string): Promise<{ report: AuditReport; engine: string }> {
  return new Promise((resolve) => {
    try {
      ...
      py.on("error", (err) => {
        // Python not available in environment (e.g. Vercel Node runtime)
        resolve({
          report: runHeuristicAudit(content, fileName),
          engine: "heuristic-fallback",
        });
...
// Master audit executor (Gemini AI -> Python Engine -> Deterministic Heuristic Fallback)
export async function performAudit(
  content: string,
  fileName?: string
): Promise<{ success: boolean; report: AuditReport; engine: string; notice?: string }> {
  const apiKey = process.env.GEMINI_API_KEY;

  // If no Gemini key, route to Python/Heuristic engine
  if (!apiKey) {
...
Return ONLY valid JSON matching this schema:
{
  ...
  try {
    ...
    const parsedReport: AuditReport = JSON.parse(cleaned);
    ...
    parsedReport.engine = "gemini-3.7-flash";
    ...
    console.warn("[Gemini API] Failed or overloaded, falling back to deterministic auditor:", geminiError);
